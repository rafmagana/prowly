...
Obtaining a users APIKEY with the prowly gem is a three step process: provider gets a token with their provider key, user approves the request at a special web URL and then provider gets user's API key with their provider key and the token.
